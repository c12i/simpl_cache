...
pub fn ttl_cache(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let call_internal_fn = if is_async {
        quote! {
            ::simpl_cache::block_on(#internal_function(#(#function_args_names),*))
        }
    } else {
        quote! {
    ...
    let output = quote! {
        #function_visibitly #async_keyword fn #function_name(#function_args) -> #function_return_type {
            ...
            thread_local! {
                static #static_var: ::std::cell::RefCell<::simpl_cache::TtlCache<String, #function_return_type>> = ::std::cell::RefCell::new(
